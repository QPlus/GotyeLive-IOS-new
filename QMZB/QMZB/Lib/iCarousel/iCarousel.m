...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gesture
{
    ...
    {        
        ...
        BOOL left = (self.scrollOffset == 0 && [(UIPanGestureRecognizer *)gesture velocityInView:gesture.view].x > 0);
        
        BOOL right = (self.scrollOffset == self.numberOfItems - 1) && ([(UIPanGestureRecognizer *)gesture velocityInView:gesture.view].x < 0);
        
        if (_edgeRecognition && (left || right)) {
            return NO;
        }
...
- (void)didPan:(UIPanGestureRecognizer *)panGesture
{
    if (_scrollEnabled)
    {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(carousel:panGestureRecognizer:)]) {
            [_delegate carousel:self panGestureRecognizer:panGesture];
        }
