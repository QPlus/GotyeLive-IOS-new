...
- (UILabel *)textLabel
{
    if (_textLabel == nil) {
        ...
        _textLabel.hidden = YES;//#lg, hide text
...
- (void)isSelected:(BOOL)selected
{
    ...
    if(selected){
        _textLabel.textColor = COLOR(25, 151, 220, 1);
    } else {
...
- (void)layoutSubviews
{
    ...
    if (self.tag == 101) {
        ...
        _itemButton.frame = CGRectMake(0, -10, self.frame.size.width, kItemButtonHeight+20);
        _textLabel.frame = CGRectMake(0, _itemButton.frame.size.height- 10, self.frame.size.width, kTextLabelHeight);
    }else {
        
        _itemButton.frame = CGRectMake(0, 0, self.frame.size.width, kItemButtonHeight + 10);
