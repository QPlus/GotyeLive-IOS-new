...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter]  addObserver:self selector:@selector(registerSeccess:) name:NotificationRegisterSeccess object:nil];
...
//注册后自动登录
- (void)registerSeccess:(NSNotification *)notification
{
    NSDictionary *dic = notification.userInfo;
    NSDictionary *userinfo = [dic objectForKey:@"userinfo"];
    _nameWindow.text = [userinfo objectForKey:@"account"];
    _passwordWindow.text = [userinfo objectForKey:@"password"];
//    [self clickLogin:nil];
}
...
- (void)initView
{
    ...
    [_nameWindow textWithleftImage:@"login_ic_username" placeName:@"手机/用户名"];
...
//        [findPassword setModalTransitionStyle:UIModalTransitionStyleFlipHorizontal];
        ...
        [self.view.window.layer addAnimation:animation forKey:nil];
        [self presentViewController:findPassword animated:NO completion:nil];
    }else {
...
//        [self.navigationController pushViewController:registeredUser animated:YES];
...
//    NSLog(@"--------");
    ...
    [_requestClient postddByByUrlPath:@"/live/Login" andParams:parameters andCallBack:^(id back) {
        ...
        if (result_type == 10000) {
            ...
            userInfo.userName = _nameWindow.text;
...
- (void)getMyLiveRoom
{
    ...
    [_requestClient postddByByUrlPath:@"/live/GetMyLiveRoom" andParams:parameters andCallBack:^(id back) {
        ...
        if (result_type == 10000) {
            ...
            AppDelegateInstance.userInfo.playRtmpUrl = [NSString jsonUtils:[dics objectForKey:@"playRtmpUrl"]];
            AppDelegateInstance.userInfo.playHlsUrl = [NSString jsonUtils:[dics objectForKey:@"playHlsUrl"]];
            AppDelegateInstance.userInfo.playFlvUrl = [NSString jsonUtils:[dics objectForKey:@"playFlvUrl"]];
